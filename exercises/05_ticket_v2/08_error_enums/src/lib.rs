...
#[derive(Debug)]
enum TicketNewError {
    TitleError { errmsg: String },
    DescriptionError { errmsg: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError { errmsg }) => {
            Ticket::new(title, "Description not provided".into(), status).unwrap()
        }
        Err(TicketNewError::TitleError { errmsg }) => panic!("{errmsg}"),
    }
}
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                errmsg: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                errmsg: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                errmsg: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                errmsg: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
